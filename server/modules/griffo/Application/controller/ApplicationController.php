    ...
    private static function select($attributes = array(), $id = 0, $json = false) {
        try {
            if (isset($_SESSION[CLIENT.LAYER])) {
                ...
                global $APP;
                $APP->setDependencyModule(array('Application'));
                $return = array();
                $application = array();
                $application = Application::all();
                $return[] = array('label' => "", 'value' => "");
                foreach ($application as $key => $value) {
                    $return[] = array('label' => $value->name, 'value' => $value->idapplication);
                }
        ...
        try {
            ...
            if (isset($_SESSION[CLIENT.LAYER])) {
                ...
                if (!empty($attributes)) {
                    ...
                    if(!Authentication::check_access_control(CUR_LAYER,'application','insert')){
                    ...
                    $APP->CON->commit();
                    $APP->activityLog("Register Application {$array['name']}");
    ...
    private static function delete($attributes = array(), $id = 0, $json = false) {
        if (is_numeric($id) && $id > 0) {
            try {
                ...
                if(!Authentication::check_access_control(CUR_LAYER,'application','delete')){
                    throw new Exception("ERROR.RECUSED.RESOURCE");
                }
    ...
    private static function update_attributes($attributes = array(), $id = 0, $json = false) {
        ...
        if (!empty($attributes) && (is_numeric($id) && $id > 0)) {
            try {
                ...
                if(!Authentication::check_access_control(CUR_LAYER,'application','update')){
                    throw new Exception("ERROR.RECUSED.RESOURCE");
                }
                ...
                $find = Application::find('all', array('conditions' => "name = '{$array['name']}' and idapplication <> {$idapplication}"));
