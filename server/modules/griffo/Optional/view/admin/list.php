<div class="view-header">{{'MODULE.OPTIONAL.TITLE.LIST' | grTranslate}}</div>
<div class="view-content" >
    <table gr-table remote="{module: 'optional', action: 'get', onelevel: true}" export-csv="GRIFFO_optionalList">
		<tr gr-repeat="(key, data) in $data">
            <td data-title="'#'" sortable="'idoptional'" filter="{'idoptional': 'text'}" header-class="col-1" class="col-1 text-center">
                {{data.idoptional}}
            </td>
            <td data-title="'LABEL.NAME' | grTranslate" sortable="'name'" filter="{'name': 'text'}" header-class="col-4" class="col-4">
                {{data.name}}
            </td>
            <td data-title="'LABEL.VALUE.MONEY' | grTranslate" sortable="'value'" filter="{'value': 'text'}" header-class="col-1">
                {{data.value | currency}}
            </td>
            <td data-title="'LABEL.DIVISIBLE' | grTranslate" sortable="'divisible'" class="col-1 text-center">
                <span class="badge badge-{{data.divisible === 1 ? 'success' : 'danger'}}">&nbsp;</span>
            </td>
            ...
            <td data-title="'BUTTON.EDIT' | grTranslate" class="col-1 text-center">
                <button class="btn btn-xs btn-table btn-primary" ng-click="grTable.fn.edit(grTable,data.idoptional,'view/admin/edit.php')"><i class="fa fa-fw fa-edit"></i></button>
            </td>
            <td data-title="'BUTTON.DELETE' | grTranslate" class="col-1 text-center">
                <button class="btn btn-xs btn-table btn-danger" ng-click="grTable.fn.delete(grTable,data.idoptional)"><i class="fa fa-fw fa-times"></i></button>
            </td>
		</tr>
    </table>
</div>
