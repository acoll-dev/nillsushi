<header class="header" ng-cloak>
    <nav class="navbar navbar-default">
            ...
            <div class="col-xs-10">
                <div class="navbar-text-header">
                    ...
                    <p class="text-primary" ng-repeat="shop in gr.shops" ng-if="shop.phone1">
                        <strong>{{shop.name}}</strong> <i class="fa fa-fw fa-phone"></i>{{shop.phone1 | phone}}
                    </p>
                </div>
        ...
        <div class="container" ng-show="GRIFFO.viewPort.width < 768">
            <div class="logo">
                <a ng-href="{{GRIFFO.curAlias}}">
                    <img class="img-responsive" ng-src="{{GRIFFO.baseUrl + GRIFFO.templatePath}}image/logo.png" />
                </a>
            </div>
            <div class="navbar-text-header">
                <p class="text-primary" ng-repeat="shop in gr.shops" ng-if="shop.phone1">
                    <strong>{{shop.name}}</strong> <i class="fa fa-fw fa-phone"></i>{{shop.phone1}}
                </p>
            </div>
        </div>
    </nav>
    ...
    <div class="user-toolbar" ng-if="GRIFFO.user" gr-affix offset-top="{xs: 87,sm: 367, md: 367, lg: 367}" ng-cloak>
        <div class="container">
            <a class="btn btn-default pull-left" ng-class="{'btn-sm': GRIFFO.viewPort.bs === 'xs'}" ng-href="{{GRIFFO.curAlias}}" ng-if="GRIFFO.filter.page.url !== 'home/'">
                ...
                <span ng-show="GRIFFO.viewPort.bs !== 'xs'">Início</span>
            </a>
            ...
            <a ng-href="{{GRIFFO.curAlias}}/user/" class="btn btn-default" ng-class="{'btn-sm': GRIFFO.viewPort.bs === 'xs'}" title="Painel do cliente">
            ...
            <button ng-click="gr.cart.submit()" class="btn btn-success" ng-class="{'btn-sm': GRIFFO.viewPort.bs === 'xs'}" title="Finalizar pedido" ng-if="gr.cart.length() > 0">
